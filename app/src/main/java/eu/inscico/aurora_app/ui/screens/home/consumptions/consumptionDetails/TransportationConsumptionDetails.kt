...
fun TransportationConsumptionDetails(
    ...
    Column(
        ...
        ListItem(
            ...
            trailingContent = {
                Text(text = "${String.format("%.1f",consumption.value)} km", color = MaterialTheme.colorScheme.onSurfaceVariant, style = MaterialTheme.typography.bodyMedium)
            }
        )
        ...
        ListItem(
            ...
            trailingContent = {
                Text(text = CalendarUtils.toDateString(consumption.transportation.dateOfTravel, "dd.MM.yyyy, HH:mm"), color = MaterialTheme.colorScheme.onSurfaceVariant, style = MaterialTheme.typography.bodyMedium)
            }
        )
        ...
        ListItem(
            ...
            trailingContent = { Text(text = stringResource(id = consumption.transportation.transportationType.getDisplayNameRes()), color = MaterialTheme.colorScheme.onSurfaceVariant, style = MaterialTheme.typography.bodyMedium) }
        )

        Divider()

        val occupancyText = when {
            consumption.transportation.publicVehicleOccupancy != null -> consumption.transportation.publicVehicleOccupancy.getDisplayName(context)
        ...
        if(occupancyText != null){
            ListItem(
                ...
                trailingContent = { Text(text =occupancyText, color = MaterialTheme.colorScheme.onSurfaceVariant, style = MaterialTheme.typography.bodyMedium) }
        )
        Divider()
        }
